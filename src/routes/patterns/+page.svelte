...
<div class="page-enter px-6 py-8 md:px-10 md:py-12 max-w-6xl mx-auto">
	...
	<div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
		<div>
			<h1 class="text-rosys-fg text-[24px] md:text-[28px] font-bold tracking-[-0.03em]">My Patterns</h1>
			<p class="text-rosys-fg-faint text-[13px] mt-1">
		...
		<div class="flex items-center gap-3">
			...
			<div class="flex bg-warm-100 rounded-xl p-0.5">
				<button
					...
					class="px-4 py-1.5 rounded-[10px] text-[12px] font-medium transition-all duration-150
						{!showAll ? 'bg-white shadow-sm text-rosys-fg' : 'text-rosys-fg-faint'}"
			...
			<div class="relative w-56">
				...
				<input
					...
					bind:value={search}
					placeholder="Search..."
					class="w-full pl-9 pr-4 py-2 rounded-xl bg-white border border-rosys-border/50 text-[13px] text-rosys-fg placeholder-rosys-fg-faint/50 focus:outline-none focus:ring-2 focus:ring-rosys-400/20 focus:border-rosys-300 transition-all"
				/>
			</div>
		</div>
	</div>
	...
	{#if !showAll && myPatterns.length === 0}
		<!-- Empty state -->
		<div class="flex flex-col items-center justify-center py-20 rosys-card">
			<div class="w-16 h-16 rounded-2xl bg-rosys-50 flex items-center justify-center mb-5">
				<Scissors class="w-7 h-7 text-rosys-300" strokeWidth={1.5} />
			</div>
			<h2 class="text-[16px] font-semibold text-rosys-fg mb-1">No patterns yet</h2>
			<p class="text-rosys-fg-faint text-[13px] text-center max-w-sm">
				Your free monthly patterns and any purchased patterns will appear here.
			</p>
			<button
				type="button"
				class="rosys-btn-primary mt-5"
				onclick={() => (showAll = true)}
	...
	{:else}
		...
		{#if !showAll && data.purchasedPatterns.length > 0}
			<div class="mb-8">
				<div class="flex items-center gap-2 mb-4">
					...
					<h2 class="text-[11px] font-semibold text-rosys-fg-muted uppercase tracking-[0.1em]">Purchased</h2>
				</div>
		...
		{#if !showAll && data.freePatterns.length > 0}
			<div class="mb-8">
				<div class="flex items-center gap-2 mb-4">
					...
					<h2 class="text-[11px] font-semibold text-rosys-fg-muted uppercase tracking-[0.1em]">Free This Month</h2>
				</div>
				<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4">
		...
		{#if displayPatterns.length === 0 && search}
			<div class="text-center py-16">
				<Scissors class="w-10 h-10 mx-auto mb-3 text-rosys-200" strokeWidth={1.5} />
...
{#snippet patternCard(pattern: typeof data.allPatterns[0], status: 'purchased' | 'free' | 'locked')}
	...
	<a
		...
		class="group rosys-card overflow-hidden transition-all duration-300
			{isLocked ? 'opacity-60 cursor-default' : 'hover:-translate-y-1'}"
	>
