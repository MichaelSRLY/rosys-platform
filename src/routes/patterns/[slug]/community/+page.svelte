...
<div class="page-enter px-6 py-8 md:px-10 md:py-12 max-w-3xl mx-auto">
	...
	<h1 class="font-[var(--font-logo)] italic text-rosys-fg text-[26px] font-light tracking-tight mb-2">Community</h1>

	<!-- Stats -->
	<div class="flex items-center gap-4 mb-8">
		<div class="flex items-center gap-1">
			{#each [1,2,3,4,5] as s}
				<Star class="w-5 h-5 {s <= Math.round(avgRating) ? 'text-amber-400 fill-amber-400' : 'text-rosys-fg/15'}" strokeWidth={1.5} />
			{/each}
		</div>
	...
	<!-- Review form -->
	{#if showForm}
		<div class="bg-rosys-card rounded-2xl border border-rosys-border/50 p-6 mb-8 shadow-sm">
			...
			<div class="flex gap-1 mb-4">
				{#each [1,2,3,4,5] as s}
					...
					>
						<Star class="w-7 h-7 {s <= (hoverRating || rating) ? 'text-amber-400 fill-amber-400' : 'text-rosys-fg/15'}" strokeWidth={1.5} />
					</button>
				{/each}
			</div>
					...
					{#each fitOptions as opt}
						<button
							...
							class="px-3 py-1.5 rounded-lg text-[12px] font-medium transition-all
								{fitNotes === opt ? 'bg-rosys-fg text-white' : 'bg-rosys-bg text-rosys-fg-muted hover:bg-rosys-bg-alt'}"
			...
			<div class="mb-4">
				...
				<textarea
					...
					class="w-full px-4 py-3 rounded-xl bg-rosys-bg border-none text-[14px] text-rosys-fg placeholder-rosys-fg-faint/40 focus:outline-none focus:ring-2 focus:ring-rosys-fg/15 resize-none"
				></textarea>
			</div>
			...
			<button
				...
				class="flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-rosys-fg text-white text-[13px] font-semibold hover:bg-rosys-fg/90 active:scale-[0.98] transition-all disabled:opacity-40"
			>
				{#if submitting}
	...
	{:else if userReview}
		<button
			type="button"
			class="text-[13px] text-rosys-fg-faint hover:text-rosys-fg mb-6 transition-colors"
	...
	<div class="space-y-4">
			...
			<div class="bg-rosys-card rounded-2xl border border-rosys-border/40 p-5 shadow-sm">
				<div class="flex items-center justify-between mb-3">
					<div class="flex items-center gap-3">
						<div class="w-8 h-8 rounded-full bg-rosys-bg-alt flex items-center justify-center">
							<span class="text-[12px] font-semibold text-rosys-fg/60">{review.username[0]?.toUpperCase()}</span>
						</div>
						<div>
					...
					<div class="flex gap-0.5">
						{#each [1,2,3,4,5] as s}
							<Star class="w-3.5 h-3.5 {s <= review.rating ? 'text-amber-400 fill-amber-400' : 'text-rosys-fg/10'}" strokeWidth={1.5} />
						{/each}
					</div>
				</div>
				{#if review.fit_notes}
					<span class="inline-block px-2.5 py-1 rounded-md bg-rosys-bg-alt text-[11px] font-medium text-rosys-fg-muted mb-2">{review.fit_notes}</span>
				{/if}
		...
		{#if reviews.length === 0}
			<div class="text-center py-12">
				<Star class="w-10 h-10 text-rosys-fg/10 mx-auto mb-3" strokeWidth={1.5} />
				<p class="text-rosys-fg-faint text-[14px]">No reviews yet — be the first!</p>
			</div>
		{/if}
	</div>
</div>
